...
exp_sweep=(
    2_steps_250728_benchmark_starters_rules_ehreact_rules_w_coreactants_aplusb_True.pk
)
...
ulimit -c 0
module purge
uv run python $script mapped_rxns=${mappings_sweep[$SLURM_ARRAY_TASK_ID]} expansion=${exp_sweep[$SLURM_ARRAY_TASK_ID]} processes=$processes